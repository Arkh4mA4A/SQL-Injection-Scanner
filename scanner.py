...
def generate_html_report(login_results, search_results, secure_results):
    ...
    total = len(login_results) + len(search_results) + len(secure_results)
...
<div class="report-body">
  <div class="summary">
    ...
    <div class="stat"><h4>Endpoints Tested</h4><div class="val">3</div></div>
    <div class="stat"><h4>Verdict</h4>
      <div class="val red" style="font-size:16px;margin-top:4px;">{"VULNERABLE" if vulns else "SECURE"}</div>
    </div>
  </div>
  {build_table(login_results, "Endpoint 1 — Vulnerable Login (/login)")}
  {build_table(search_results, "Endpoint 2 — User Search (/search)")}
  {build_table(secure_results, "Endpoint 3 — Secure Login (/secure) — Parameterized Queries")}
  <div style="margin-top:32px;padding:20px 24px;background:#ebf8f0;border-radius:8px;border:1px solid #9ae6b4;">
    <strong style="color:#276749;">How the Secure Login prevents SQL Injection:</strong>
    <p style="color:#2d6a4f;font-size:13px;margin-top:8px;">
      Instead of building the SQL query using f-strings (which allows user input to become
      part of the command), the secure endpoint uses <strong>parameterized queries</strong>.
      The <code>?</code> placeholder tells the database engine to treat the input as a literal
      string value — never as executable SQL. All payloads above returned "Invalid credentials"
...
""")
    ...
    # ── Scan the secure login endpoint to show the contrast ──────────────────
    print_section("Scanning: Secure Login (/secure) — should block all payloads")
    print("  This endpoint uses parameterized queries.")
    print("  The same payloads that bypassed /login should all FAIL here.\n")
    secure_s = requests.Session()
    secure_results, secure_vulns = scan_endpoint(
        secure_s, SECURE_URL, LOGIN_PAYLOADS, "username", "Secure Login"
    )

    # ── Final summary ────────────────────────────────────────────────────────
    total_vulns = login_vulns + search_vulns
    total_tests = len(LOGIN_PAYLOADS) + len(SEARCH_PAYLOADS) + len(LOGIN_PAYLOADS)

    print_section("FINAL SUMMARY")
    print(f"  Endpoints scanned    : 3  (Vulnerable Login, User Search, Secure Login)")
    print(f"  Total payloads sent  : {total_tests}")
    print(f"  Vulnerable endpoints : /login and /search")
    if total_vulns:
        print(Fore.RED  + Style.BRIGHT + f"  Vulnerabilities found: {total_vulns}")
    if secure_vulns == 0:
        print(Fore.GREEN + f"  Secure endpoint      : /secure blocked all {len(LOGIN_PAYLOADS)} payloads")
        print(Fore.GREEN +  "  Parameterized queries successfully prevent SQL injection.")
    print(Fore.RED  + Style.BRIGHT + "\n  VERDICT: Vulnerable endpoints are VULNERABLE TO SQL INJECTION")
    print(Fore.GREEN + Style.BRIGHT +   "  VERDICT: Secure endpoint is PROTECTED\n")

    print(f"  Generating HTML report ...")
    path = generate_html_report(login_results, search_results, secure_results)
